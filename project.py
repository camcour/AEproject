...
def main():
    ...
    tableau_prop_inc.rename(index = {un_texte_inconnu[0]:'inconnu'}, inplace=True)

    #tranpose() par souci d'esthétique, et pour bien aller avec le sens de corr
    tableau_moyennes_fr_an_inc = pd.concat([tableau_prop_inc, tableau_fr, tableau_an], sort=True).transpose()

    correlation_fr_inc = tableau_moyennes_fr_an_inc['fr'].corr(tableau_moyennes_fr_an_inc['inconnu'])
    correlation_an_inc = tableau_moyennes_fr_an_inc['an'].corr(tableau_moyennes_fr_an_inc['inconnu'])
